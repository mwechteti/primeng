...
export class SearchFormComponent {
  ...
  constructor(fb: FormBuilder, private router: Router) {
    this.searchForm = fb.group({
      regNumber: [ Validators.maxLength(15)],
...
export class ShowOnFormInvalidStateMatcher implements ErrorStateMatcher {
  isErrorState(control: FormControl | null, form: FormGroupDirective | null): boolean {
    return !!((control?.invalid) ?? (form?.hasError('minLessThanMax')));
  }
}
...
function withoutEmptyValues(object: any): any {
  return Object.keys(object as {}).reduce((queryParams: any, key) => {
    if (object[key]) { queryParams[key] = object[key]; }
    return queryParams as unknown;
  }, {});
}
